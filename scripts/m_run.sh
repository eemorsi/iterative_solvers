...
CMD="./solver -solver ${SOLVER}  -log ${LOG}  -precond_log ${P_LOG}  -niter 1  -maxit ${MAXIT}  -tol 1e-17  -emtx ${E_MTX} -mtx ${MTX}"
...
do 
       if [[ $DEBUG -eq 0 ]]; then
              ...
              /opt/nec/ve/bin/mpirun -ve ${VE} -np ${NP} ${CMD}
       else
              export OMP_NUM_THREADS=${NTH} 
              /opt/nec/ve/bin/mpirun -ve ${VE} -np ${NP} /usr/bin/xterm -hold -e /opt/nec/ve/bin/gdb --args ${CMD}
       fi
done