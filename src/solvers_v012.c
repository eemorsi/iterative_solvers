...
#pragma omp parallel
   ...
   /* Parse command line */
   {
      ...
      while (arg_index < argc)
      {
         ...
         else if (strcmp(argv[arg_index], "-emtx") == 0)
         {
            arg_index++;
            mtx_ext = atoi(argv[arg_index++]);
         }
   ...
   if (myid == 0)
   {
      ...
      // printf("nrows %d, ncol %d \t nrows %d, ncol %d\n", t_coo_matrix.nrows, t_coo_matrix.ncolumns, coo_matrix.nrows, coo_matrix.ncolumns);
   ...
   if (myid != 0)
   {
      ...
      if (x_flag == 1)
         x_sol_vals = (double *)aligned_alloc(align_size, coo_matrix.ncolumns * sizeof(double));
   ...
   /* Set the rhs values and the solution to zero */
   {
      ...
      rhs_values = (double *)calloc(iupper[1] - ilower[1] + 1, sizeof(double));
      x_values = (double *)calloc(iupper[1] - ilower[1] + 1, sizeof(double));
      // rows = (HYPRE_Int *)calloc(local_size, sizeof(HYPRE_Int));

      for (i = ilower[1]; i <= iupper[1]; i++)
      {
         if (rhs_flag != 1)
            rhs_values[i - ilower[1]] = 1;
         // if (rhs_flag == 1)
         //    rhs_values[i - ilower[1]] = rhs[i];
         if (x_flag != 1)
            x_values[i] = 0.0;
      }
      ...
      if (rhs_flag == 1)
         HYPRE_IJVectorSetValues(x, local_size, rows, x_sol_vals);
      else
   ...
   */
   ...
   if (solver_id == 0)
   {
      ...
      HYPRE_BoomerAMGSetMaxLevels(solver, 8);         /* maximum number of levels */
   ...
   else if (solver_id == 50)
   {
      ...
      HYPRE_PCGSetRelChange(solver, 1);            /* use the two norm as the stopping criteria */
      // HYPRE_PCGSetStopCrit(solver, 4);            /* use the two norm as the stopping criteria */
      // HYPRE_PCGSetRelChange(solver, 1);
...
/* Now set up the AMG preconditioner and specify any parameters */
      ...
      HYPRE_CGNRSetPrecond(solver, (HYPRE_PtrToSolverFcn)HYPRE_BoomerAMGSolve,(HYPRE_PtrToSolverFcn)HYPRE_BoomerAMGSolve,
                                  (HYPRE_PtrToSolverFcn)HYPRE_BoomerAMGSetup, precond);
   ...
   else if (solver_id == 13)
   {
      ...
      HYPRE_ParCSRGMRESSetPrecond(solver, (HYPRE_PtrToParSolverFcn)HYPRE_ParCSRPilutSolve,
